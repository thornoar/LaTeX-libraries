...
path fullgraph (picture pic=currentpicture, real f(real) = exp, real x1=-1, real x2=1, real xmargin=0.5, real ymargin=0.7, bool drawaxis=true, bool displaynumbers=true, bool displayleftnumber=true, bool displayrightnumber=true, pen axispen=currentpen, pen plotpen=currentpen+blue+cyan, bool smooth=true, bool drawdashes=true, bool drawleftdash=true, bool drawrightdash=true, bool draweq=true, align align=E, align leftnumberalign = S, align rightnumberalign=S, string graphlabel = "$y = e^x$", int n=1000, arrowbar axisarrow = Arrow(SimpleHead), bool drawzero = true)
{
    if (drawaxis)
    {
        int n = 10;
...
void drawcoords (picture pic = currentpicture, int x = 10, int y = 10, real ticksize = .5, int step = 1, pen textpen = fontsize(12pt))
{
    draw(pic, (-x, 0)--(x,0));
    draw(pic, (0, -y)--(0,y));
...
void drawxyz (picture pic = currentpicture, real s, pen p = currentpen, arrowbar3 arrow = Arrow3())
{
...
void drawimplicit (picture pic=currentpicture, real f(real, real), pair a, pair b)
{
...
void addclip (picture pic=currentpicture, path p, real scale=1, pair shift=(5,0))
{
    picture g;
    g=pic;
...
path[] tickequalsegment (pair a, pair b, int n = 1, real length = .5, real distance = .05)
{
    path[] res = new path[n];
    pair t;
    if (a.y == b.y) {t = (0,1);}
...
picture drawwfunction (string[] l, int[] t = new int[], int mode, pen p = blue+.5)
{
    picture q;
